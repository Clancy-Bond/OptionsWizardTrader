            ...
            if len(contract_parts) >= 3:
                ...
                if expiry_date:
                    ...
                    if 'symbol' in main_contract:
                        ...
                        if strike_price:
                            summary += f"({strike_price}) options expiring {expiry_date}, purchased {timestamp_str if timestamp_str else '2025-04-14'}.\n\n"
                        else:
                            summary += f"({contract_parts[1]}) options expiring {expiry_date}, purchased {timestamp_str if timestamp_str else '2025-04-14'}.\n\n"
                    else:
                        summary += f"({contract_parts[1]}) options expiring {expiry_date}, purchased {timestamp_str if timestamp_str else '2025-04-14'}.\n\n"
                else:
                    # Fallback to just the second part if we couldn't parse a proper date
                    summary += f"({contract_parts[1]}) options expiring soon, purchased {timestamp_str if timestamp_str else '2025-04-14'}.\n\n"
            else:
