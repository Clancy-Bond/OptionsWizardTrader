"""
Fix the formatting of the stop loss response to match the screenshot example.
This will modify the handle_stop_loss_request method to properly format the response.
"""

import re

def fix_response_formatting():
    """Update the handle_stop_loss_request method to match the desired formatting."""
    try:
        with open('discord_bot.py', 'r') as f:
            content = f.read()
        
        # Save a backup just in case
        with open('discord_bot.py.formatting_backup', 'w') as f:
            f.write(content)
        ...
        try:
            ...
            if recommendation:
                ...
                # Main STOP LOSS RECOMMENDATION section
                ...
                embed.add_field(
                    name=f"{chart_emoji} {horizon_text} {chart_emoji}",
        ...
        print("Successfully updated the formatting of handle_stop_loss_request")
        return True
        
    except Exception as e:
        print(f"Error applying formatting fix: {e}")
        return False

if __name__ == "__main__":
    fix_response_formatting()