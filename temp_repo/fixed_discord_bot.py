...
class OptionsBot:
    def enforce_buffer_limits(self, stop_loss, current_price, option_type, days_to_expiration):
        ...
        # For calls: 
        # - stop_loss should be support level ABOVE current_price*(1-max_buffer%)
        ...
        if option_type.lower() == 'call':
            ...
            # For puts:
            # - stop_loss should be resistance level BELOW current_price*(1+max_buffer%)
            # - if stop_loss > max_buffer_stop, then we're too far from current price
            if stop_loss > max_buffer_stop:
                buffer_exceeded = True
        ...
        if buffer_exceeded:
            ...
            print(f"No buffer enforcement needed: Technical level {stop_loss:.2f} is within {max_buffer_percentage:.1f}% buffer limit")
        ...
        try:
            ...
            try:
                ...
                if option_type.lower() == 'call':
                    ...
                    technical_buffer_percentage = abs((current_price - stop_loss) / current_price * 100)
                else:
                    # For puts, percentage is how far above current price
                    technical_buffer_percentage = abs((stop_loss - current_price) / current_price * 100)
