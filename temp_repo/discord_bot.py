...
class OptionsBotNLP:
    ...
    def extract_info(self, text):
        ...
        # Extract percentage change (like "up 5%" or "down 3.5%") - do this first to avoid confusion with dollar amounts
        percent_change_pattern = r'(?:up|down|rise|fall|increase|decrease|move)(?:\s+by)?\s+([\d.]+)%'
        percent_change_match = re.search(percent_change_pattern, text.lower())
        if percent_change_match:
            try:
                change_percentage = float(percent_change_match.group(1))
                # Determine if it's positive or negative
                direction = -1 if re.search(r'down|fall|decrease', text.lower()) else 1
                info['percent_change'] = change_percentage * direction
            except ValueError:
                pass
                
        # Extract dollar amount change (like "up $5" or "down $3.50") - but only if no percentage was found
        if 'percent_change' not in info:
            # Modified pattern to explicitly exclude percentages by adding a negative lookahead (?!%)
            dollar_change_pattern = r'(?:up|down|rise|fall|increase|decrease|move)(?:\s+by)?\s+\$?([\d.]+)(?!\s*%)(?:\s+dollars|\s+bucks)?'
            dollar_change_match = re.search(dollar_change_pattern, text.lower())
            if dollar_change_match:
                try:
                    change_amount = float(dollar_change_match.group(1))
                    # Determine if it's positive or negative
                    direction = -1 if re.search(r'down|fall|decrease', text.lower()) else 1
                    info['dollar_change'] = change_amount * direction
                except ValueError:
                    pass
...
class OptionsBot:
    def enforce_buffer_limits(self, stop_loss, current_price, option_type, days_to_expiration):
        ...
        # Safety check - if inputs are None or invalid, return defaults
        if stop_loss is None or current_price is None or current_price <= 0:
            print("WARNING: Invalid inputs to enforce_buffer_limits")
            # Default to 5% buffer if we can't calculate
            # Set reasonable defaults if current_price is None
            if current_price is None:
                current_price = 100.0  # Arbitrary reasonable value
                print("WARNING: Using default current price of $100")
                
            if option_type.lower() == 'call':
                return current_price * 0.95, 5.0, True
            else:
                return current_price * 1.05, 5.0, True
        ...
        # Calculate what the max buffer stop would be
        if option_type.lower() == 'call':
            # For calls, lower is better (support levels)
            # The minimum allowable stop loss (furthest from current price)
            max_buffer_stop = current_price * (1 - max_buffer_percentage/100)
        else:
            # For puts, higher is better (resistance levels)
            # The maximum allowable stop loss (furthest from current price)
            max_buffer_stop = current_price * (1 + max_buffer_percentage/100)
            
        # Check if the stop loss exceeds maximum buffer
        buffer_exceeded = False
        
        # For calls: 
        # - stop_loss should be support level ABOVE max_buffer_stop
        # - if stop_loss < max_buffer_stop, then we're too far from current price
        if option_type.lower() == 'call':
            # If stop loss is LOWER than max buffer stop, it exceeds allowed buffer
            if stop_loss < max_buffer_stop:
                buffer_exceeded = True
                print(f"CALL option stop loss ${stop_loss:.2f} exceeds max buffer ${max_buffer_stop:.2f}")
                print(f"Technical level buffer is {current_buffer_percentage:.2f}%, max allowed is {max_buffer_percentage:.2f}%")
        else:
            # For puts:
            # - stop_loss should be resistance level BELOW max_buffer_stop
            # - if stop_loss > max_buffer_stop, then we're too far from current price
            if stop_loss > max_buffer_stop:
                buffer_exceeded = True
                print(f"PUT option stop loss ${stop_loss:.2f} exceeds max buffer ${max_buffer_stop:.2f}")
                print(f"Technical level buffer is {current_buffer_percentage:.2f}%, max allowed is {max_buffer_percentage:.2f}%")
            
        if buffer_exceeded:
            # Apply the maximum allowed buffer instead
            adjusted_stop_loss = max_buffer_stop
            ...
            if adjustment_percentage > 0.01:
                print(f"Enforcing buffer: Adjusting stop loss from ${stop_loss:.2f} to ${adjusted_stop_loss:.2f} to respect {max_buffer_percentage:.1f}% buffer limit for {days_to_expiration} DTE")
                print(f"Current price: ${current_price:.2f}, adjustment amount: {adjustment_percentage:.2f}% of price")
            
            # Return adjusted stop loss, max buffer percentage, and flag that enforcement was applied
            return adjusted_stop_loss, max_buffer_percentage, True
        else:
            # Current buffer is within limits, no need to adjust
            # Return original stop loss, max buffer percentage, and flag that no enforcement was needed
            print(f"No buffer enforcement needed: Stop level ${stop_loss:.2f} is within required buffer limit of {max_buffer_percentage:.1f}%")
            print(f"Technical level buffer is {current_buffer_percentage:.2f}%, max allowed is {max_buffer_percentage:.2f}%")
    ...
    def __init__(self):
        ...
        self.approved_channels = ["ai-options-calculator", "╟💠-ai-options-bot"]  # Updated to include new channel
...
IMPORTANT: I only respond when directly mentioned in the #╟💠-ai-options-bot channel or its threads."""
    ...
    async def handle_message(self, message):
        ...
        # STRICTLY enforce channel restriction to ONLY approved channels or threads within those channels
        if channel_name not in self.approved_channels:
        ...
        if has_mention:
            ...
            try:
                if info['request_type'] == 'option_price':
                    ...
                    default_embed.set_footer(text="I only respond when directly mentioned in the #╟💠-ai-options-bot channel or its threads")
                ...
                try:
                    ...
                    if closest_option is not None:
                        ...
                        embed = discord.Embed(
                            ...
                            description=f"**Stock Price:** ${current_price:.2f}",
                            color=embed_color
                        )
                        ...
                        # Check if we need to calculate option price at a target stock price
                        # Either from a dollar change or percentage change
                        target_price = None
                        target_description = ""
                        target_date = None
                        
                        # Look for target date in info
                        from datetime import datetime, timedelta
                        if 'target_date' in info and info['target_date']:
                            target_date = datetime.strptime(info['target_date'], '%Y-%m-%d')
                        
                        # Check if there's a specified date like "by May 1st"
                        date_pattern = r'by\s+(?:(?:this|next|coming)\s+)?(?:(?:mon|tues|wednes|thurs|fri|satur|sun)(?:day)?|(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\.?\s+\d{1,2}(?:st|nd|rd|th)?(?:[,]?\s+\d{2,4})?)'
                        target_date_match = re.search(date_pattern, message.content.lower())
                        
                        if target_date_match and not target_date:
                            target_date_text = target_date_match.group(0).replace('by ', '')
                            # Try to parse the date
                            try:
                                # Use similar date parsing logic as in parse_relative_dates
                                now = datetime.now()
                                
                                # Handle month + day format
                                month_day_pattern = r'(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\.?\s+(\d{1,2})'
                                month_day_match = re.search(month_day_pattern, target_date_text.lower())
                        ...
                        # If no target date specified, show a message asking for one
                        if ('dollar_change' in info or 'percent_change' in info) and not target_date:
                            # Create an error embed for missing timeframe
                            missing_embed = discord.Embed(
                                title="Missing Timeframe",
                                description=f"What timeframe are you asking about? To calculate what your {ticker} ${closest_option['strike']:.2f} {info['option_type']}s expiring {selected_expiration} would be worth if the stock " + 
                                           f"{'went up' if ('dollar_change' in info and info['dollar_change'] > 0) or ('percent_change' in info and info['percent_change'] > 0) else 'went down'} " +
                                           f"{'$' + str(abs(info['dollar_change'])) if 'dollar_change' in info else str(abs(info['percent_change'])) + '%'}, " +
                                           f"I need to know which date you're interested in due to theta decay.",
                                color=0xFF9900  # Orange for warning
                            )
                            missing_embed.add_field(
                                name="Example",
                                value=f"What would my {ticker} ${closest_option['strike']:.2f} {info['option_type']}s be worth if the stock " +
                                     f"{'went up' if ('dollar_change' in info and info['dollar_change'] > 0) or ('percent_change' in info and info['percent_change'] > 0) else 'went down'} " + 
                                     f"{'$' + str(abs(info['dollar_change'])) if 'dollar_change' in info else str(abs(info['percent_change'])) + '%'} " +
                                     "by May 1st?",
                                inline=False
                        ...
                        # Get option greeks to help with price estimation
                        option_greeks = None
                        try:
                            from option_calculator import get_option_greeks
                            option_greeks = get_option_greeks(stock, selected_expiration, closest_option['strike'], info['option_type'])
        ...
        try:
            ...
            if isinstance(expiration_date, str):
                try:
                    ...
                    formatted_date = f"{date_obj.year}-{month_str}-{date_obj.day:02d}"
                except:
                ...
                formatted_date = f"{expiration_date.year}-{month_str}-{expiration_date.day:02d}"
            ...
            try:
                ...
                # Get theta with a reasonable default value for option decay if not available
                # Default theta varies based on days to expiration
                raw_theta = option_data.get('theta', -0.01)
                
                # Ensure theta is negative (representing decay) and has a minimum meaningful value
                # For 0 DTE options, use a higher minimum to ensure visible hourly decay
                if days_to_expiration == 0:
                    # For same-day expiration, ensure we have a more significant hourly decay
                    # At least -0.04 daily (-0.0017 hourly) to show visible hourly decay (0.2% per hour)
                    theta = min(raw_theta, -0.04) if raw_theta <= 0 else -0.04
                else:
                    # For non-zero DTE, use normal minimum
                    theta = min(raw_theta, -0.01) if raw_theta <= 0 else -0.01
            ...
            # Calculate stop loss level - special handling for 0DTE options
            try:
                # For 0DTE options, use fixed 15% stop loss
                buffer_enforcement_override = False
                fixed_stop_option_price = None
                fixed_loss_percentage = None
                
                # Initialize stop_loss_data variable to prevent reference errors
                stop_loss_data = {}
                
                if days_to_expiration == 0:
                    print("0DTE option detected - using fixed 15% stop loss recommendation")
                    
                    # Use the fixed 0DTE stop loss calculator
                    try:
                        import option_price_calculator as opc
                        
                        # Calculate the fixed 15% stop loss for 0DTE options
                        fixed_stop_data = opc.calculate_fixed_0DTE_stop_loss(
                            current_stock_price=current_price,
                            current_option_price=option_price,
                            strike_price=strike_price,
                            option_type=option_type,
                            fixed_loss_percentage=15.0,  # Use a fixed 15% loss
                            implied_volatility=implied_volatility if implied_volatility > 0 else 0.3
                        )
                        
                        technical_stop = fixed_stop_data['stop_stock_price']
                        fixed_stop_option_price = fixed_stop_data['stop_option_price']
                        fixed_loss_percentage = fixed_stop_data['actual_loss_percentage']
                        
                        print(f"Fixed 15% stop loss for 0DTE: Stock price at ${technical_stop:.2f}, option price at ${fixed_stop_option_price:.2f} ({abs(fixed_loss_percentage):.1f}% loss)")
                        
                        # Create a minimal stop_loss_data structure for compatibility with the rest of the code
                        stop_loss_data = {
                            'trade_horizon': 'scalp',
                            ...
                            'scalp': {
                                'level': technical_stop,
                                'time_horizon': 'scalp',
                                'method': 'fixed-percentage-0DTE'
                            }
                        }
                        
                        # Since we're applying a fixed methodology, no buffer enforcement is needed
                        buffer_enforcement_override = True
                        
                    except Exception as e:
                        print(f"Error calculating fixed 0DTE stop loss: {e}")
                        buffer_enforcement_override = False
                        
                        # Fall back to regular technical analysis if the fixed calculation fails
                    ...
                    # For non-0DTE options, use the standard technical analysis
                    stop_loss_data = get_stop_loss_recommendations(
                        ticker_symbol, current_price, option_type, expiry_str
                    )
                
                # Extract the stop loss level (if not already set by fixed 0DTE calculator)
                if days_to_expiration != 0 or not buffer_enforcement_override:
                    # Extract the stop loss level - first try to find in nested structures
                    technical_stop = None
                    if "level" in stop_loss_data:
                        technical_stop = stop_loss_data.get("level")
                        print(f"DETAILED LOGS: Found technical stop level {technical_stop} in direct level key")
                    elif stop_loss_data.get("swing") and "level" in stop_loss_data.get("swing", {}):
                        technical_stop = stop_loss_data.get("swing", {}).get("level")
                        print(f"DETAILED LOGS: Found technical stop level {technical_stop} in swing nested structure")
                    elif stop_loss_data.get("primary") and "level" in stop_loss_data.get("primary", {}):
                        technical_stop = stop_loss_data.get("primary", {}).get("level")
                        print(f"DETAILED LOGS: Found technical stop level {technical_stop} in primary nested structure")
                    else:
                        # No level found in any structure, use default fallback
                        technical_stop = current_price * 0.95 if option_type == "call" else current_price * 1.05
                        print(f"DETAILED LOGS: No level found in stop_loss_data, using fallback level {technical_stop}")
                
                # Check for numpy.float64 type and convert to standard Python float
                if hasattr(technical_stop, 'item') and callable(getattr(technical_stop, 'item')):
                    technical_stop = technical_stop.item()  # Convert numpy.float64 to standard Python float
                
                stop_loss = technical_stop  # Initialize stop_loss with the technical level
                
                # IMPORTANT: Let's display the technical buffer first BEFORE enforcing limits
                # Calculate the buffer percentage from the technical stop loss level
                if option_type.lower() == 'call':
                    # For calls, percentage is how far below current price
                    technical_buffer_percentage = abs((current_price - technical_stop) / current_price * 100)
                else:
                    # For puts, percentage is how far above current price
                    technical_buffer_percentage = abs((technical_stop - current_price) / current_price * 100)
                
                # Now check if we need to enforce buffer limits and get the max buffer
                # Returns (adjusted_stop_loss, max_buffer_percentage, enforced)
                # IMPORTANT: We must pass technical_stop, not stop_loss, to correctly check against buffer limits
                adjusted_stop_loss, max_buffer, buffer_enforced = self.enforce_buffer_limits(technical_stop, current_price, option_type, days_to_expiration)
                
                # Calculate max buffer stop level for logging (same calculation as in enforce_buffer_limits)
                if option_type.lower() == 'call':
                    max_buffer_stop = current_price * (1 - max_buffer/100)  # Lower limit for calls
                else:
                    max_buffer_stop = current_price * (1 + max_buffer/100)  # Upper limit for puts
                    
                # Print all values for debug
                print(f"DEBUG COMPARISONS: current_price=${current_price}, technical_stop=${technical_stop}")
                print(f"DEBUG COMPARISONS: max_buffer_stop=${max_buffer_stop}, adjusted_stop_loss=${adjusted_stop_loss}")
                
                # CRITICAL CHANGE: Now we'll actually use the adjusted_stop_loss if the buffer was enforced
                # If buffer wasn't enforced, we'll keep the original technical support level
                if buffer_enforced:
                    # Buffer was enforced, use the adjusted stop loss and max buffer for display
                    stop_loss = adjusted_stop_loss  # Update the actual stop_loss we'll use
                    stop_loss_buffer_percentage = max_buffer  # Use max buffer percentage for display
                else:
                    # No enforcement needed, use the technical level and its percentage
                    stop_loss_buffer_percentage = technical_buffer_percentage
                
                # Log technical and buffer levels for clarity
                # Get level from either direct key or from nested dictionaries based on trade_horizon
                original_stop_loss = None
                if "level" in stop_loss_data:
                    original_stop_loss = stop_loss_data.get("level")
                elif stop_loss_data.get("swing") and "level" in stop_loss_data.get("swing", {}):
                    original_stop_loss = stop_loss_data.get("swing", {}).get("level")
                elif stop_loss_data.get("primary") and "level" in stop_loss_data.get("primary", {}):
                    original_stop_loss = stop_loss_data.get("primary", {}).get("level")
                
                # Check if we found a level and log appropriately
                if original_stop_loss is not None:
                    print(f"DETAILED LOGS: Technical level from get_stop_loss_recommendations: ${original_stop_loss:.2f}")
                else:
                    print(f"DETAILED LOGS: Technical level from get_stop_loss_recommendations: None")
                    print(f"DETAILED LOGS: Using technical_stop instead: ${technical_stop:.2f}")
                    
                print(f"DETAILED LOGS: Current technical buffer percentage: {technical_buffer_percentage:.2f}%")
                print(f"DETAILED LOGS: Max buffer level: ${max_buffer_stop:.2f} ({max_buffer:.2f}%)")
                print(f"DETAILED LOGS: Final stop level to be displayed: ${stop_loss:.2f}")
                print(f"DETAILED LOGS: Buffer was enforced: {buffer_enforced}")
                print(f"DETAILED LOGS: Display percentage will be: {stop_loss_buffer_percentage:.2f}%")
                ...
                if current_price is not None and stop_loss is not None and option_price is not None and delta is not None:
                    ...
                    if needs_full_bs:
                        try:
                            ...
                            option_stop_price = opc.calculate_option_price_at_stop(
                                current_option_price=option_price,
                                current_stock_price=current_price,
                                stop_stock_price=stop_loss,
                                strike_price=strike_price,
                            ...
                            if option_type.lower() == 'call':
                                option_price_change = price_change * max(0.2, abs(delta))
                            else:
                                option_price_change = -price_change * max(0.2, abs(delta))
                ...
                # Add all stop loss info in a single field with special formatting for 0DTE
                if days_to_expiration == 0 and fixed_stop_option_price is not None:
                    # Always display a fixed 15% loss for 0DTE options for consistency
                    embed.add_field(
                        name="",
                        value=f"• Stock Price: ${current_price:.2f}\n• Option Price: ${option_price:.2f}\n• 🛑 Stop loss: ${stop_loss:.2f} ({stop_loss_buffer_percentage:.1f}% {'below' if option_type.lower() == 'call' else 'above'} price)\n• 🛑 Option Stop: ${fixed_stop_option_price:.2f} (a 15.0% loss)",
                        inline=False
                    )
                else:
                    # Standard display for other options
                    embed.add_field(
                        name="",
                        value=f"• Stock Price: ${current_price:.2f}\n• Option Price: ${option_price:.2f}\n• 🛑 Stop loss: ${stop_loss:.2f} ({stop_loss_buffer_percentage:.1f}% {'below' if option_type.lower() == 'call' else 'above'} price)\n• 🛑 Option Stop: ${option_stop_price:.2f} (a {abs(loss_percentage):.1f}% loss)",
                        inline=False
                    )
                ...
                if days_to_expiration <= 1:
                    ...
                    if days_to_expiration == 0:
                        # Simpler message for 0DTE (emphasize the 30-min rule)
                        embed.add_field(
                            name="⚡ SCALP ⚡",
                            value=f"• Scalp trades are generally only meant to be held for 15-30 minutes max due to the quick theta decay\n• Exit immediately if your profit target is reached or time limit exceeded",
                            inline=False
                        )
                    else:
                        # Standard message for 1DTE
                        embed.add_field(
                            name="⚡ SCALP ⚡",
                            value=f"• Scalp trades are generally only meant to be held for 15-30 minutes max due to the quick theta decay",
                            inline=False
                    ...
                    embed.add_field(
                        ...
                        value=f"• 🛑 Stop loss: ${stop_loss:.2f} ({stop_loss_buffer_percentage:.1f}% {'below' if option_type == 'call' else 'above'} price)\n• Based on stock's volatility (2x ATR)",
                        inline=False
                    )
                
                else:
                    # LONG-TERM trade - minimal display per user request
                    # No separate section for long-term trades
                    pass
                ...
                if days_to_expiration >= 0 and theta is not None:  # Changed > to >= to include 0 DTE options
    ...
    async def handle_unusual_activity_request(self, message, info):
        ...
        try:
            ...
            # Create main content with bullet points
            main_content = ""
            if largest_flow > 0:
                ...
                # Timestamp has been removed as requested
                
                # Add overall flow summary at the bottom
            ...
            # Remove footer timestamp per user request
            # No footer needed
