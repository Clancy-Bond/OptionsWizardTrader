...
# Import LEAPS-specific weekly-based stop loss module for 60+ DTE
...
def get_scalp_stop_loss(stock, current_price, option_type, days_to_expiration=None, trade_type=None):
    ...
    try:
        ...
        if option_type.lower() == 'call':
            ...
            # ATR buffer removed
            atr_buffer = 0.0001  # Near-zero buffer
    ...
    try:
        ...
        if option_type.lower() == 'call':
            ...
            if valid_supports:
                ...
                technical_buffer = 0  # ATR buffer removed
                ...
                stop_loss_atr = current_price - (0.0001)  # ATR buffer removed, using price-based only
...
def get_longterm_stop_loss(stock, current_price, option_type, days_to_expiration=None, trade_type=None):
    ...
    try:
        ...
        if option_type.lower() == 'call':
            ...
            stop_loss = current_price - (0.0001)  # ATR buffer removed, using price-based only
...
def get_stop_loss_recommendation(stock, current_price, option_type, expiration=None):
    ...
    try:
        ...
        # For true LEAPS (180+ DTE), prioritize weekly-based stop loss approach
        leaps_weekly_stop = None
        if days_to_expiration is not None and int(days_to_expiration) >= 180:
            leaps_weekly_stop = get_leaps_weekly_stop_loss(stock, current_price, option_type, days_to_expiration)
            print(f"LEAPS weekly-based stop loss analysis completed: {leaps_weekly_stop is not None}")

        # If not a LEAPS or LEAPS analysis failed, try pattern-based enhanced system
        enhanced_stop = None
        if not leaps_weekly_stop:
        ...
        # If LEAPS weekly-based stop loss was found, prioritize it for long-term options
        if leaps_weekly_stop:
            result["leaps_weekly"] = leaps_weekly_stop
            
            # For LEAPS, always prioritize the weekly-based stop as primary
            if trade_horizon == "longterm" or (days_to_expiration is not None and int(days_to_expiration) >= 180):
                result["primary"] = leaps_weekly_stop
                print(f"Using LEAPS weekly-based stop loss as primary recommendation")
        # If enhanced stop loss was found, add it to the result and prioritize it (if LEAPS not available)
        elif enhanced_stop:
        ...
        if days_to_expiration is not None:
            ...
            # Include LEAPS weekly-based stop loss if it's available and applicable
            if days_to_expiration is not None and int(days_to_expiration) >= 180 and "leaps_weekly" in result:
                filtered_result["leaps_weekly"] = result["leaps_weekly"]
                print("Including leaps_weekly stop loss recommendation in filtered result")
...
def get_stop_loss_recommendations(ticker_symbol, current_price, option_type, expiration_date=None):
    ...
    try:
        ...
        # If LEAPS weekly-based stop loss exists, include it in the adapted result
        if "leaps_weekly" in result:
            adapted_result["leaps_weekly"] = result["leaps_weekly"]
            print(f"DEBUG: LEAPS weekly-based stop loss found and included in adapted result")
