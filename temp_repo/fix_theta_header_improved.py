...
def fix_discord_bot():
    """Update discord_bot.py to include the date in the field name with proper f-string syntax"""
    try:
        with open('discord_bot.py', 'r', encoding='utf-8') as file:
            content = file.read()
        
        # Fix all instances with proper escaping for nested quotes in f-strings
        # Example: name="⚠️ THETA DECAY PROJECTION ⚠️",
        # We want to change it to: name=f"⚠️ THETA DECAY PROJECTION TO ({info['expiration']}) ⚠️",
        
        # Replace all instances
        instances = [
            # Pattern for theta decay field in handle_option_price_request and handle_stop_loss_request
