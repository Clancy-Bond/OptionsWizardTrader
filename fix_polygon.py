"""
Script to restore polygon_integration.py from backup and then 
implement moneyness detection properly
"""

def fix_polygon_integration():
    # Define the backup content that has the correct function but without modifications
    backup_content = """
...
"""
    ...
    # Read the current content to find the start and end of the function
    with open('polygon_integration.py', 'r') as f:
        content = f.read()
    
    # Find start and end of the function
    start_function = content.find("def get_simplified_unusual_activity_summary(ticker):")
    if start_function == -1:
        print("Could not find the function in the file")
        return False
    
    # Find the end of the function (next function definition)
    next_function = content.find("def ", start_function + 1)
    if next_function == -1:
        # If no next function, use end of file
        end_function = len(content)
    else:
        end_function = next_function
    
    # Replace the function
    new_content = content[:start_function] + backup_content + content[end_function:]
    
    # Write the modified content back to the file
    with open('polygon_integration.py', 'w') as f:
        f.write(new_content)
    
    print("Successfully fixed polygon_integration.py with dynamic moneyness detection")
    return True

if __name__ == "__main__":
    print("Applying fix to polygon_integration.py...")
    success = fix_polygon_integration()
    print(f"Fix completed: {'Successfully' if success else 'Failed'}")